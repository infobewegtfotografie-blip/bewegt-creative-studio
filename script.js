(function(){
  ...
  /* ═══ MARKET TOGGLE (Africa / Europe pricing) ═══ */
  (function initMarketToggle(){
    var marketButtons = Array.from(document.querySelectorAll('.market-btn'));
    var marketGrids = Array.from(document.querySelectorAll('[data-market-grid]'));
    var currencyPanel = document.querySelector('.currency-panel');
    if (!marketButtons.length || !marketGrids.length) return;

    function setMarket(market){
      marketGrids.forEach(function(grid){
        grid.hidden = grid.getAttribute('data-market-grid') !== market;
      });
      marketButtons.forEach(function(btn){
        btn.classList.toggle('is-active', btn.dataset.market === market);
      });
      // Currency selector only makes sense for the Africa (FCFA-based) grid
      if (currencyPanel) currencyPanel.style.display = market === 'africa' ? '' : 'none';
      localStorage.setItem('bewegtMarket', market);
    }
    ...
    var savedMarket = localStorage.getItem('bewegtMarket') || 'africa';
  ...
  // Base : 1 FCFA (XOF) converti vers chaque devise
  const FALLBACK_RATES = {
    EUR: 1 / 655.957,   // 1 EUR ≈ 655.957 FCFA (parité fixe XOF/EUR)
    USD: 1 / 610,
    GBP: 1 / 770,
  ...
  function showRateIndicator(source, rates) {
    ...
    var eurDisp = Math.round(1 / rates.EUR).toLocaleString('fr-FR');
    var usdDisp = Math.round(1 / rates.USD).toLocaleString('fr-FR');
  ...
  (function fetchLiveRates() {
    ...
    function applyRates(rates, source) {
      CURRENCIES.forEach(function(code) {
        if (rates[code] && currencyConfig[code]) {
          currencyConfig[code].rate = rates[code];
        }
      });
      ...
      showRateIndicator(source, rates);
    }
    ...
    fetch('https://open.er-api.com/v6/latest/XOF')
      ...
      .then(function(data) {
        if (data && data.rates && data.rates.EUR && data.rates.USD) {
          ...
          try {
            localStorage.setItem(CACHE_KEY, JSON.stringify({ rates: rates, ts: Date.now() }));
          } catch(e) {}
          applyRates(rates, 'live');
        } else {
