(function(){
  ...
  const savedLang = safeStorage.get('bewegtLang', 'en');
  applyLang(savedLang);
  ...
  document.querySelectorAll('.lang-btn').forEach(btn => btn.addEventListener('click', () => {
    if (articleVersions) {
      ...
      if (cible && cible !== location.pathname) {
        safeStorage.set('bewegtLang', btn.dataset.lang);
        location.href = cible;
        return;
      }
    }
  ...
  (function() {
    ...
    nlForm.addEventListener('submit', function(e) {
      ...
      fetch('/', { method: 'POST', body: data })
        .then(function(r) {
          if (r.ok) {
            var dict = (typeof translations !== 'undefined' && translations[safeStorage.get('bewegtLang', 'en')]) || {};
  ...
  (function initPricingEngine(){
    ...
    let activeService = singleService ? root.getAttribute('data-pricing-engine') : (safeStorage.get('bewegtService', 'photo'));

    function quoteLabel(){
      const lang = safeStorage.get('bewegtLang', 'en');
    ...
    function render(){
      ...
      tierCards.forEach(card => {
        ...
        if (list) {
          const lang = PRICING_INCLUSIONS[safeStorage.get('bewegtLang', 'en')]
            ? safeStorage.get('bewegtLang', 'en')
            : 'en';
  ...
  /* ═══ FIX: show prev/next buttons immediately, not only when playing ═══ */
  (function initSoundSkipVisibility(){
  ...
  function setSoundLabel(){
    ...
    if(soundVolume){
      ...
      soundVolume.setAttribute('aria-label', volumeLabels[document.documentElement.lang] || volumeLabels.en);
    }
    if(soundPrev && soundNext){
      ...
      const skipLabel = skipLabels[document.documentElement.lang] || skipLabels.en;
  ...
  if(siteSound){
    ...
    siteSound.addEventListener('ended', () => {
      if(isWelcomeIntro){
        const lang = document.documentElement.lang || 'en';
        safeSession.set(`bewegtWelcomeHeard-${lang}`, '1');
        isWelcomeIntro = false;
  ...
  document.querySelectorAll('[data-rail-carousel]').forEach((carousel) => {
    const slides = Array.from(carousel.querySelectorAll('.rail-promo'));
    const previous = carousel.querySelector('[data-rail-prev]');
    const following = carousel.querySelector('[data-rail-next]');
    const status = carousel.querySelector('[data-rail-status]');
    ...
    const start = () => {
      ...
      if(!window.matchMedia('(prefers-reduced-motion: reduce)').matches){
        timer = window.setInterval(() => display(current + 1), 6000);
      }
    };

    previous?.addEventListener('click', () => { display(current - 1); start(); });
    following?.addEventListener('click', () => { display(current + 1); start(); });
